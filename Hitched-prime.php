...
<div class="one">	
 ...
 <div class="container">
			...
			if(mysqli_connect_errno())
			{
				...
				if($gender=="Female")
				{
						$data=mysqli_query($con,"select v.img , m.name, v.age ,v.religion ,v.qual, v.ms,v.kids , v.ht , v.wt, v.wrkp ,v.city ,v.sal ,v.email  from project.main1 m  , project.viewtable v and project.checktable c where m.$column like '%$q%' and m.email=v.email and v.gender='male' and c.contact=v.contact and c.checkval=1");
...
<div class="container">
	<?php
		...
		$contact;
		if($gender=="Female")
		{	$qry="select v.img, m.name , v.age ,v.religion ,v.qual, v.ms , v.ht , v.wt, v.wrkp ,v.city ,v.sal ,v.email ,v.contact from project.main1 m, project.viewtable v , project.checktable c where c.checkval=1 and v.contact=c.contact and v.email=m.email and v.gender='male';";
						...
						else
						{
							while($row=mysqli_fetch_assoc($res))
							{
								...
								echo "
									<a href=\"./User/Profile.php?email=".$ch."\">See Profile</a>
									</div>
									</div>
									</div>";
		...
		{
					$qry="select v.img, m.name , v.age ,v.religion , v.ms , v.ht , v.wt, v.wrkp ,v.city ,v.sal , v.email from project.main1 m, project.viewtable v , project.checktable c where c.checkval=1 and v.contact=c.contact and v.email=m.email and v.gender='Female';";
						$res=mysqli_query($con,$qry);
						if(!$res)
						{
							echo("retrival failed".$res -> error);
